...
let http_ver_hdrs = {
	'x-quic': 'h3', 'x-firefox-http3': 'h3', 'x-firefox-spdy': 'h2' }
let http_ver_desc = {
	'h3': 'http/3', 'h2': 'http/2',
...
let http_ver_check = e => {
	...
	let proto = Object.fromEntries(
		(e.responseHeaders || []).map( h =>
			[http_ver_hdrs[h.name.toLowerCase()], h.value] ) )
	if (proto.h3) return tab_proto.set(e.tabId, 'h3')
	if (proto.h2) return tab_proto.set(
		e.tabId, proto.h2.match(/^h2/) ? 'h2' : 'spdy' )
	tab_proto.set(e.tabId, 'old') }
...
let set_tab_icon = tab_id => {
	...
	browser.pageAction.setTitle({tabId: tab_id, title: http_ver_desc[ver]}) }
